  ...
  <v-container fluid grid-list-xl>
    ...
    <v-layout row wrap v-if="infiniteScrollPosts">
      <v-flex xs12 sm6 v-for="post in infiniteScrollPosts.posts" :key="post.id">
        <v-card hover>
          <v-card-media @click.native="goToPost(post.id)" :src="post.imageUrl" height="30vh" lazy></v-card-media>
            ...
            <v-card-text v-show="showPostCreator" class="grey lighten-4">
              <v-list-tile avatar>
                ...
                <v-list-tile-content>
                  ...
                  <v-list-tile-subtitle class="font-weight-thin">Added {{ formatCratedDate(post.createdDate) }}</v-list-tile-subtitle>
                </v-list-tile-content>
