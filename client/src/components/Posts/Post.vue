        ...
        <v-card hover>
          <v-card-title>
            ...
            <v-btn @click="handleToggleLike" large icon v-if="user">
              <v-icon large :color="checkIfPostLiked(getPost.id) ? 'red' : 'grey'">favorite</v-icon>
            </v-btn>
  ...
  export default {
    ...
    methods: {
      ...
      checkIfPostLiked(postId) {
        ...
        if (
          this.userFavorites &&
          this.userFavorites.some(fave => fave.id === postId)
        ) {
          this.postLiked = true;
          return true;
        } else {
      ...
      checkIfOwnMessage(message) {
        return this.user && this.user.id === message.messageUser.id;
      }
    }
  };
</script>
